...
// Reset cooldown when opening a new chat
function openChat(receiverId, receiverName, chatId) {
  ...
  loadChatMessages(receiverId, 15, null, true); // Load 15 initial messages
...
let lastLoadTime = 0;
const LOAD_COOLDOWN = 30000; // 10 seconds
...
function setupChatScrollLoading(receiverId) {
  ...
  const scrollHandler = createScrollThrottle(function () {
    ...
    if (
      ...
      // Load messages
      loadChatMessages(receiverId, 20, earliestMessageId)
        .then(() => {
...
function renderAllChatMessages() {
  ...
  console.log(
    "✅ Rendered all messages in chronological order (oldest to newest)"
  );
}
...
function sortUsersByActivity(users) {
  console.log("🔄 Sorting users by activity:", {
    conversationOrder,
    userHasMessages,
    lastMessageTimes,
  });
  ...
  return users.sort((a, b) => {
    ...
    console.log(`Comparing users ${a.nickname} vs ${b.nickname}:`, {
      aHasMessages,
      bHasMessages,
      aTime,
      bTime,
    ...
    // Neither has message history: alphabetical
    console.log(`Neither has messages, sorting alphabetically`);
